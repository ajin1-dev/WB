...
target         <- 0.05 ##meta_indicator$target_value
...
progress_results <-  suppressWarnings(
  ...
  code_col       = "iso3c",
  year_col       = "date",
  startyear_data = 2001, ## startyear_data,
  endyear_data   = 2023, ## endyear_data,
  eval_from      = 2000, ## meta_indicator$start_prog_eval,
  eval_to        = 2024, ## meta_indicator$end_prog_eval,
  future         = TRUE,
  target_year    = 2150,
  speed          = TRUE,
  percentiles    = FALSE, ## TRUE,
  sequence_pctl  = seq(20, 80, 20),
  sequence_speed = c(0.25, 0.5, 1, 2, 4),
  best           = "high", ## best,
#  min            = meta_indicator$min,
#  max            = meta_indicator$max,
  support        = 1, ## meta_indicator$support,
  granularity    = 0.1 ## meta_indicator$granularity
))
